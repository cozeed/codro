            ...
            {items.map((item, index) => {
              ...
              return (
                <ComposerPrimitive.Unstable_TriggerPopoverItem
                  ...
                  item={item}
                  index={index}
                  className="hover:bg-accent focus:bg-accent data-highlighted:bg-accent flex w-full cursor-pointer flex-col items-start gap-0.5 px-3 py-1.5 text-start transition-colors outline-none"
                >
                  ...
                  {item.description && (
                    <span className="text-muted-foreground ms-5.5 text-[11px] leading-tight">{item.description}</span>
                  )}
                </ComposerPrimitive.Unstable_TriggerPopoverItem>
              );
            })}
            {items.length === 0 && <div className="text-muted-foreground px-3 py-2 text-sm">{emptyLabel}</div>}
          </div>
        </div>
      )}
    </ComposerPrimitive.Unstable_TriggerPopoverItems>
...
const ComposerTriggerPopoverImpl: FC<ComposerTriggerPopoverProps> = ({
  ...
  return (
    ...
    >
      ...
      <Items iconMap={iconMap} fallbackIcon={fallbackIcon} backLabel={backLabel} emptyLabel={emptyItemsLabel} />
    </ComposerPrimitive.Unstable_TriggerPopover>
  );
};
