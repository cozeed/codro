  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        toast.success(t("sign.signUpSuccess", { name: value.name }));
        onSuccess?.();
      } catch (error) {
        console.error("Error signing up", error);
        toast.error(t("sign.signUpFail"));
      }
    },
  });
    ...
    >
      <div>
        <form.Field
          ...
          children={(field) => (
            <>
              <Label htmlFor={field.name}>{t("sign.fullName")}</Label>
      ...
      <div>
        <form.Field
          ...
          children={(field) => (
            <>
              <Label htmlFor={field.name}>{t("sign.confirmPassword")}</Label>
              <div className="relative mt-1">
                <Input
                  className="peer ps-10 focus-visible:ring-1"
                  id={field.name}
      ...
      <form.Subscribe
        ...
        children={([canSubmit, isSubmitting]) => (
          <Button type="submit" disabled={!canSubmit} className="mt-2 h-10">
            {isSubmitting ? <Spinner /> : t("sign.signUp")}
          </Button>
        )}
      />
    </form>
