...
export function MacOS({ className, ...props }: HTMLProps<HTMLDivElement>) {
  ...
  const [isAltKeyPressed, setIsAltKeyPressed] = useState(true);
  ...
  return (
    <div
      ...
      {...props}
    >
      <ThemeToggle className="aspect-square h-3 w-3 cursor-default content-center items-center justify-center self-center rounded-full border border-black/[.12] bg-[#28c93f] text-center text-black/60 hover:bg-[#28c93f] active:bg-[#1e9930] active:text-black/60 dark:border-none" />
      {isTauriApp() ? (
        <>
          <Button
            onClick={closeWindow}
