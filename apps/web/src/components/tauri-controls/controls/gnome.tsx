...
export function Gnome({ className, ...props }: HTMLProps<HTMLDivElement>) {
  ...
  return (
    <div className={cn("mr-[10px] h-auto items-center space-x-[13px]", className)} {...props}>
      <ThemeToggle className="m-0 aspect-square h-6 w-6 cursor-default rounded-full bg-[#dadada] p-0 text-[#3d3d3d] hover:bg-[#d1d1d1] active:bg-[#bfbfbf] dark:bg-[#373737] dark:text-white dark:hover:bg-[#424242] dark:active:bg-[#565656]" />
      {isTauriApp() ? (
        <>
          <Button
          ...
          >
            <Icons.minimizeWin className="h-[2.25] w-[2.25]" />
          </Button>
          <Button
          ...
          >
            {!isWindowMaximized ? (
              ...
              <Icons.maximizeRestoreWin className="h-[2.25] w-[2.25]" />
            )}
          </Button>
          <Button
            onClick={closeWindow}
